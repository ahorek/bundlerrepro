...
class Project < ActiveRecord::Base
  ...
  has_many :issues, :dependent => :destroy_async
  has_many :issue_changes, :through => :issues, :source => :journals
  has_many :versions, :dependent => :destroy_async
  belongs_to :default_version, :class_name => 'Version'
  belongs_to :default_assigned_to, :class_name => 'Principal'
  has_many :time_entries, :dependent => :destroy_async
  has_many :queries, :dependent => :delete_all
  has_many :documents, :dependent => :destroy_async
  has_many :news, lambda {includes(:author)}, :dependent => :destroy_async
  ...
  has_many :repositories, :dependent => :destroy_async
  has_many :changesets, :through => :repository
  has_one :wiki, :dependent => :destroy_async
